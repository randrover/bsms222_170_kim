...
```{r}
p1 <- data.frame(x_hat) %>% ggplot(aes(x = x_hat)) + geom_histogram(binwidth = 0.005, color = "black", fill = "gray")
p2 <- data.frame(x_hat) %>% ggplot(aes(x = x_hat, y = x_hat)) + geom_point() + geom_abline(intercept = 0, slope = 1)
plot_grid(p1, p2)
```
...
p3 <- data.frame(x_hat) %>% ggplot(aes(x = x_hat)) + geom_histogram(binwidth = 0.005, color = "black", fill = "gray")
p4 <- data.frame(x_hat) %>% ggplot() + geom_qq(aes(sample = x_hat))+ geom_qq_line(aes(sample = x_hat))
plot_grid(p3, p4)


# B가 10, 100, 1000... normal distribution 같음..
```
...
dim(inside)
d <- t(inside)
d <- as.data.frame(d) %>% mutate(start = V1 - 1.96 * V2, end = V1 + 1.96 * V2)
#이렇게 between이 아니라 식을 써 줘야 한다..
d <- d%>% mutate(hit_inside = ifelse(start<p & end>p, "YES", "NO")) %>% rename(x_hat = V1, se_hat = V2)

```

```{r}
d<-d %>% mutate(n = seq(1, B))

d[1:100,] %>% ggplot(aes(color = hit_inside)) + geom_point(aes(x=n, y=x_hat)) +
geom_segment( aes(x=n, xend=n, y=start, yend=end)) +
  coord_flip()


d[1:100,] %>% ggplot(aes(x=x_hat, y=n, color = hit_inside)) + geom_point() +
...
# 샘플링 function
kelk <- function(){
  x <- sample(heights_w$height, size = N, replace = FALSE)
  x_hat <- mean(x)
  se_hat <- sd(x)/sqrt(N)
  return(c(x_hat, se_hat))
}
...
d <- as.data.frame(d) %>% mutate(n = seq(1,B)) %>% rename(x_hat = V1, se_hat = V2) %>%
  ...
  mutate(hit_inside = ifelse(start<p_w & end>p_w, "YES", "NO"))
...
intervals <- replicate(B, {
  ...
  x_hat <- mean(x)
  se_hat <- sqrt(x_hat * (1 - x_hat) / n)
  start = x_hat - 1.96 * se_hat
  end = x_hat + 1.96 * se_hat
  hit = ifelse(between(p, x_hat - 1.96 * se_hat, x_hat + 1.96 * se_hat), "hit", "fail")
  c(start, end, hit)
})

head(intervals[1,])

intervals_d <- data.frame(n = 1:10000, start = intervals[1,], end = intervals[2,], hit = intervals[3,])


head(intervals_d, 100) %>% ggplot(aes(x = n, ymin = start, ymax = end, color = inside)) +
  geom_errorbarh()
...
#the odds ratio of losing under pressure
...
exp(c_i)
```
1 is not included. (1.00부터도 아닌 듯. (a,b)같은 ? < <)
...
Q3 means p-vaule is smaller than 0.05, but Q1 shows that the p-value is larger than 0.05.

Q3~> This is a slightly different p-value than that with the Chi-square test. This is because we are using a different asymptotic approximation to the null distribution.
...
```{r}
two_by_two_2 <- two_by_two %>%
  mutate(Olympics = Olympics*2, N_Olympics = N_Olympics*2) 

two_by_two_2 %>% select(-results) %>%
  chisq.test() %>% .$p.value
```

```{r}
odds_Olympics <- with(two_by_two_2, (Olympics[2]/sum(Olympics)) / (Olympics[1]/sum(Olympics)))

odds_N_Olympics <- with(two_by_two_2, (N_Olympics[2]/sum(N_Olympics)) / (N_Olympics[1]/sum(N_Olympics)))
...
exp(c_i_2)
2*(1 - pnorm(log_or, 0, se_2))
```

**0.00544228** & **0.004216133**
