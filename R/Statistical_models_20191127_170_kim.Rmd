...
```{r}
polls %>% group_by(pollster) %>% ggplot(aes(x= pollster, y = spread)) + geom_point(size = 0.5) +geom_boxplot() + coord_flip()
```
...
```{r}
aa <- polls %>% 
  group_by(pollster) %>% summarize(avg = mean(spread), s = sd(spread), N = n())

est_m <- aa$avg[2] - aa$avg[1]
s_hat <- with(aa, sqrt(s[2]^2/N[2] + s[1]^2/N[1]))

c(est_m - 1.96 * s_hat, est_m + 1.96 * s_hat)
```

**0.03851006  0.06607328**
...
polls %>% group_by(pollster) %>% 
  summarize(avg = mean(spread), 
            se = sd(spread) / sqrt(sum(samplesize))) %>%
  group_by(pollster) %>% summarize(se)

polls %>% group_by(pollster) %>% 
  summarize(avg = mean(spread), 
            se = sd(spread)/sqrt(min(samplesize))) %>%
  group_by(pollster) %>% summarize(se)
