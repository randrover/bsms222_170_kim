...
# Chapter 10 Visualizing data distributions
...
(작은 그룹, 즉 몇 개의 단어 같은 걸로 여러 데이터 구분 짓는 것 말하는 듯.)

Two simple examples are sex (male or female) and regions (Northeast, South, North Central, West). Some categorical data can be ordered even if they are not numbers per se, such as spiciness (mild, medium, hot). In statistics textbooks, ordered categorical data are referred to as ordinal data.

Example of numerical data are population sizes, murder rates, and heights. Some numerical data can be treated as ordered categorical. We can further divide numerical data into continuous and discrete. Continuous variables are those that can take any value, such as heights, if measured with enough precision.
...
(discrete하면 각각의 수가 별개의 것이 되어? ordinal..순서처럼 되는? 음? 보통은 ordinal이란 단어를 적은 수의 그룹의 variable에 쓰나 봄. 그룹 내의 원소는 많은.)
...
(그에 반해 여러 그룹, 각가의 원소는 적은 경우는 discrete numerical variabels)
...
(그러니까 담배값으로 어림잡으면 한 갑, 두 갑 이렇게 갑 개수마다 그만큼 피는 많은 사람이 속해 있는데. 갑이 아닌 실제 담배 개수로 하면 12개 피는 몇 사람, 13개 피는 몇 사람 이렇게 더 세분화되어 각각의 개수마다 포진된 사람 수는 확 줄어든다는 것인 듯.)
...
In our case study, while several students reported a height of 68 inches, only one student reported a height of `68.503937007874` inches and only one student reported a height `68.8976377952756` inches. We assume that they converted from 174 and 175 centimeters respectively.
...
Similar to what the frequency table does for categorical data, the CDF defines the distribution for numerical data. From the plot, we can see that 16% of the values are below 65, since $F(66) = 0.164$, or that 84% of the values are below 72, since $F(72) = 0.841$, etc.. In fact, we can report the proportion of values between any two heights, say $a$ and $b$, by computing $F(a) - F(b)$. This means that if we send this plot above to ET, he will have all the information needed to reconstruct the entire list. Paraphrasing the expression “a picture is worth a thousand words”, in this case, a picture is as informative as 812 numbers.
(F(66)이면 a가 66일 때, 66 이하의 숫자가 전체의 몇 퍼센트냐. 하는 것.)
...
Histograms are much preferred because they greatly facilitate answering such questions. Histograms sacrifice just a bit of information to produce plots that are much easier to interpret.
...
(히스토그램은 x축이 numeric, barplot은 categorical이로군?)
...
(히스토그램, 그러나 bin 사이즈 즉 interval이 매우매우 작다는 뜻인 듯.)
...
(interval 경계에서 뚝 떨어지는데, 그 부분의 마지막 숫자를 더 가까운 interval 경계로 흡수시켜 그래프를 smooth 하게 만든다는 듯. 꺾이는 부분에서 왼쪽이 더 높은데 오른쪽에 흡수시키면 그래프가 더욱 완만해지겠지.)
...
We often see the average and standard deviation used as summary statistics: a two-number summary! To understand what these summaries are and why they are so widely used, we need to understand the normal distribution.
...
(mean과 SD만 있어도 SD따라 가상의 그래프 그릴 수 있다는 것인가)
...
First let’s define the theoretical quantiles for the normal distribution. In statistics books we use the symbol $Φ(x)$ to define the function that gives us the probability of a standard normal distribution being smaller than $x$.

So, for example, $Φ(-1.96) = 0.025$ and $Φ(1.96) = 0.975$.
...
The inverse functino $Φ^(-1)(0.975) = 1.96$. In R, we can evaluate the inverse of $Φ$ using the `qnorm` function.
...
1. Define a vector $m$ proportions $p1, p2, ..., pm$.
2. Define a vector of quantiles $q1, ..., qm$ for your data for the proportions $p1, ..., pm$. We refer to these as the _sample quantiles_.
...
We have noticed what we didn’t expect to see. If we look at other female height distributions, we do find that they are well approximated with a normal distribution. So why are our female students different?
...
```{r}
w <- filter(heights, heights$sex == "Female")
female_percentiles <- quantile(w$height, pp)
m <- filter(heights, heights$sex == "Male")
male_percentiles <- quantile(m$height, pp)
cbind(female_percentiles, male_percentiles)
data.frame(cbind(female_percentiles, male_percentiles))
```
...
```{r}
mean(79 < x & x <= 81)
pnorm(81 , mean = mean(x), sd = sd(x)) - pnorm(79 , mean = mean(x), sd = sd(x))
```
...
```{r}
(150 / (pnorm(80, 69, 3, lower.tail = FALSE) * 1000000000)) * 100
